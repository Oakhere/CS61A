...
class Player:
    ...
    def debate(self, other):
        win_prob = max(0.1, self.popularity / (self.popularity + other.popularity))
        if win_prob > random():
            self.popularity += 50
        else:
            self.popularity -= 50
...
class VirFib():
    ...
    # This is quite interesting. Notice that "self" and "next_virfib" are switching.
    def next(self):
        if self.value == 0:
            next_virfib =  VirFib(1)
        else:
            next_virfib = VirFib(self.prev_value + self.value)
        next_virfib.prev_value = self.value
        return next_virfib
