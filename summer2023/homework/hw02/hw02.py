...
def waves(n):
    ...
    def helper(n, count, prev):
        ...
        if prev < curr and curr > next:
            return helper(rest, count + 1, curr)  # Increment count, move to the next digit
        
        # Detect a trough: negative to positive transition
        if prev > curr and curr < next:
            return helper(rest, count - 1, curr)  # Decrement count, move to the next digit
        
        if next == curr:
            return helper(rest, count, prev)
        # If none of the transitions occur, just continue to the next digit
